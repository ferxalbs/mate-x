...
* Added deterministic objective assertion compilation for scoped forbidden-pattern absence, required replacement presence, allowed declarations or compatibility stubs, file state, explicit exclusions, and complete repository-scope coverage without parsing assistant prose or arbitrary tool output.
...
* Scheduled repository objective verification after initial inspection, after mutation, and after resume when repository identity changes; stale, cross-workspace, contract-mismatched, scope-mismatched, and incomplete evidence is rejected and regenerated with the same evidence IDs propagated through finalization, persistence, Evidence Packs, Ship Proof input, and diagnostics.
...
* Restored a separate repository-grounded assistant response after canonical finalization, kept the structured outcome card compact, and automatically collapsed completed activity into an evidence-backed summary while preserving the append-only trace, manual expansion, and expanded running or approval-required states.
* Preserved terminal presentation across conversation resume by retaining bounded objective assertions and per-check validation evidence, keeping the collapsed activity summary visible even when trace events are trimmed, restoring the expandable trace when present, and showing passed focused tests exactly once in the natural response and outcome evidence row only when typed validation evidence proves them.
* Prevented unavailable provider synthesis from overriding complete typed objective proof for a zero-mutation run, so a verified preexisting repository state finishes as `completed` and `already_satisfied`; unproven objectives and changed runs retain their stricter failure or partial semantics.
* Scheduled explicitly requested, repository-authorized focused tests for completely verified no-op objectives through the existing validation planner and runner, so test evidence no longer depends on model tool support while post-mutation-only typecheck and other checks remain correctly not applicable.
...
* Added live-shape regressions for already-satisfied repositories, performed migrations, remaining prohibited calls, allowed deprecated declarations, partial coverage, passing tests without objective proof, preexisting changes, stale evidence, provider-independent prose, terminal response composition, activity collapse and expansion, resume hydration, and model tool unavailability.
...
* Added regression coverage for no-op completion, changed-but-unverified work, validation-only blocking, fallbacks, high-risk checks, toolchain capabilities, stale persistence, provider independence, authorization, trace projection, and Evidence Pack parity.
