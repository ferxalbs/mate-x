...
- **No Mixing Tailwind Utility Colors with CSS Glass System (STRICT):** Never combine inline Tailwind background/opacity/blur utility classes (e.g. `bg-popover/95`, `bg-popover/80`, `bg-panel/70`, `backdrop-blur-*`) on elements carrying `data-slot` hooks (`data-slot="select-popup"`, `data-slot="menu-popup"`, `data-slot="popover-popup"`, `data-slot="combobox-content"`) or `.mate-glass-float`. Delegate ALL background fills, translucency, and blur calculations to `src/index.css` and CSS theme variables (`--overlay-glass-tint`, `--overlay-glass-blur`, `--control`) to preserve visual coherence between blur and solid modes.
