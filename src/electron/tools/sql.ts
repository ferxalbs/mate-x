...
const SQL_INJECTION_PATTERNS = [
  {
    ...
    regex: /\.query\s*\(\s*`.*?\$\{.*?\}.*?`\s*\)/gs,
  },
  {
    name: "Concatenated Query",
    regex: /\.query\s*\(\s*['"].*?['"]\s*\+\s*.*?\)/g,
  },
  {
    name: "Unsafe Find (NoSQL)",
    regex: /\.find\s*\(\s*\{\s*.*?\$\{.*?\}.*?\}\s*\)/gs,
  },
...
export const sqlAuditTool: Tool = {
  name: "sql_audit",
  description:
    "Scans for SQL and NoSQL injection vulnerabilities, specifically identifying insecure query construction.",
  parameters: {
    type: "object",
    properties: {
      path: {
    ...
    try {
      ...
      for (const file of files) {
        ...
        for (const pattern of SQL_INJECTION_PATTERNS) {
          ...
          if (matches) {
            findings.push(
              `${file}: Detected potential ${pattern.name} (${matches.length} matches)`,
            );
          }
        }
      }
