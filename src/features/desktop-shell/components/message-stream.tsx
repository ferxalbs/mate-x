...
import type { ChatMessage, EvidencePack, MessageArtifact, ToolEvent } from '../../../contracts/chat';
...
function EvidencePackCard({ evidencePack }: { evidencePack: EvidencePack }) {
  return (
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {evidencePack.toolsUsed.map((tool) => (
            <span key={tool.name} className="rounded-md border border-border/60 bg-background/45 px-2 py-1 text-[10px] text-muted-foreground">
              {tool.name}{typeof tool.count === 'number' ? ` (${tool.count})` : ''}
            </span>
          ))}
        </div>
      ) : null}
