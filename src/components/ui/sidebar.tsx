...
function SidebarTrigger({
  ...
  style,
  ...props
}: Omit<React.ComponentProps<typeof Button>, "className" | "render" | "style"> & {
  className?: string;
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        ...
        {...props}
      >
        {showSidebarIcon ? <HugeiconsIcon icon={SidebarLeftIcon} className="size-3.5" /> : <HugeiconsIcon icon={SidebarLeftIcon} className="size-3.5" />}
...
function SidebarRail({
  ...
  const stopResize = React.useCallback(
    (pointerId: number) => {
      ...
      if (resolvedResizable?.storageKey && typeof window !== "undefined") {
        setLocalStorageItem(resolvedResizable.storageKey, resizeState.width, numberCodec);
      }
  ...
  React.useEffect(() => {
    ...
    const storedWidth = getLocalStorageItem(resolvedResizable.storageKey, numberCodec);
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "-translate-x-1/2 group-data-[side=left]:-right-4 absolute inset-y-0 z-20 hidden w-4 transition-colors duration-[var(--motion-press)] ease-[var(--ease-out)] after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=right]:left-0 sm:flex [[data-collapsible=offcanvas][data-state=collapsed]_&]:pointer-events-none",
...
function SidebarGroupLabel({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "flex h-8 shrink-0 items-center rounded-lg px-2 font-medium text-sidebar-foreground text-xs outline-hidden ring-ring transition-opacity duration-[var(--motion-press)] ease-[var(--ease-out)] focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full cursor-pointer items-center gap-2 overflow-hidden rounded-lg p-2 text-left text-sm outline-hidden ring-ring transition-[background-color,color] duration-[var(--motion-press)] ease-[var(--ease-out)] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pe-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
  {
    defaultVariants: {
      size: "default",
      variant: "default",
