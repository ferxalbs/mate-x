...
1. Pick the narrowest control that matches the user's intent (see decision guide).
2. Apply it with a `rules` subcommand (edits your `doctor.config.*` or `package.json#reactDoctor` in place, preserving other fields and formatting).
3. Validate the change did what they wanted:
